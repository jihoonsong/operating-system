...
/* Returns true if any bits in a bitmap with the name of ARGV[0]
   between ARGV[1] and ARGV[1] + ARGV[2],  exclusive, are set to true,
   and false otherwise. */
static void
execute_bitmap_any (const int argc, const char *argv[])
{
  ...
  printf (bitmap_any (entry->bitmap, start, cnt) ? "true\n" : "false\n");
}
...
/* Returns the number of bits in a bitmap with the name of ARGV[0]
   between ARGV[1] and ARGV[1] + ARGV[2], exclusive, that are set to
   ARGV[3]. */
static void
...
/* Toggles the bit numbered ARGV[1] in a bitmap with the name of ARGV[0]. */
static void
...
execute_bitmap_mark (const int argc, const char *argv[])
{
  ...
  bitmap_mark (entry->bitmap, idx);
}

/* Returns true if no bits in a bitmap with the name of ARGV[0]
   between ARGV[1] and ARGV[1] + ARGV[2], exclusive, are set to true,
   and false otherwise.*/
static void
...
/* Finds and returns the starting index of the first group of ARGV[2]
   consecutive bits in a bitmap with the name of ARGV[0] at or after
   ARGV[1] that are all set to ARGV[3].
...
execute_bitmap_scan (const int argc, const char *argv[])
{
  ...
  printf ("%zu\n", bitmap_scan (entry->bitmap, start, cnt, value));
}
